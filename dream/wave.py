...
###################################################Adaptive Optics######################################################
#read the influence function from a file
def read_influence_func(file_name):
    ...
    #Reshaping the array
    influence_waves_pV = np.reshape(rinfluence_waves_pV, (1077, 18)) 

    y_m = datay_m[742:1819]

    return y_m, influence_waves_pV

# add two rows in influence function 
def augment_influence_func(y_m, influence_waves_pV):
    a,b = np.shape(influence_waves_pV)
    row1 = np.zeros((a,1))+0.01
    row2 = y_m/np.max(y_m)*0.05

    # adding two rows for piston ad tilt, so that they are dummy variables in the matrix inversion
    h = np.concatenate((np.concatenate((influence_waves_pV,row1),axis=1),row2),axis=1)
